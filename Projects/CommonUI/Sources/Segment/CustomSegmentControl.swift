...
public class CommonHistorySegmentedControl: UISegmentedControl {
    ...
    public convenience init(frame: CGRect, types: [String]) {
    ...
    public override func draw(_ layer: CALayer, in ctx: CGContext) {
        ...
        let separtorPoint = calculateWidth(for: 3)
        let separtorPointOne = calculateWidth(for: 2)
        let separtorPointTwo = calculateWidth(for: 1)
        addSeparatorLine(x: separtorPoint, y: layer.bounds.origin.y, height: yHeight, ctx: ctx)
